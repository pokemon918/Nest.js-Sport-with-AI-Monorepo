...
export class EditWorkoutModal implements OnInit {
  constructor(
  ...
  public setWorkoutFormGroup(): void {
    this.workoutFormGroup = new FormGroup({
      id: new FormControl(this.currentForm.id),
      ...
      imageURL: new FormControl(this.currentForm.imageURL, Validators.required),
    });
    this.setImageURL();
  }
  ...
  public async updateWorkout(): Promise<any> {
    this.workoutService
      .editWorkout(this.workoutFormGroup.getRawValue())
      .subscribe(
        (res: any) => {
  ...
  public async editWorkout(): Promise<void> {
    this.isLoading = true;
    if (
